...
def main(_alpha = 0.03):
    ...
    graph_name = 'FDR_pvalues' + name_ + '.pdf'
    ...
    for geneset in range(len(_genesets)):
        ...
        # DO THINGS FOR FISHER EXACT TEST
            ...
            FDR_count_random = []
            FDR_count_SV_ordering = []
            FDR_count_ordering_normal = []
            FDR_count_ordering_normal_rescaled = []
            FDR_count_ordering_artificial = []
            FDR_count_ordering_artificial_rescaled = []
            for max_accepted in range(1,int(len(p_random))):

                # random
                y=multipletests(pvals=p_random[:max_accepted], alpha=alpha, method="fdr_bh")
                FDR_count_random.append(len(y[1][np.where(y[1]<alpha)]))

                # sv
                y=multipletests(pvals=p_SV_ordering[:max_accepted], alpha=alpha, method="fdr_bh")
                FDR_count_SV_ordering.append(len(y[1][np.where(y[1]<alpha)]))

                # po
                y=multipletests(pvals=p_ordering_normal[:max_accepted], alpha=alpha, method="fdr_bh")
                FDR_count_ordering_normal.append(len(y[1][np.where(y[1]<alpha)]))

                # por
                y=multipletests(pvals=p_ordering_normal_rescaled[:max_accepted], alpha=alpha, method="fdr_bh")
                FDR_count_ordering_normal_rescaled.append(len(y[1][np.where(y[1]<alpha)]))

                # ao
                y=multipletests(pvals=p_ordering_artificial[:max_accepted], alpha=alpha, method="fdr_bh")
                FDR_count_ordering_artificial.append(len(y[1][np.where(y[1]<alpha)]))

                # aor
                y=multipletests(pvals=p_ordering_artificial_rescaled[:max_accepted], alpha=alpha, method="fdr_bh")
                FDR_count_ordering_artificial_rescaled.append(len(y[1][np.where(y[1]<alpha)]))


            y=multipletests(pvals=p_random[:max_accepted], alpha=alpha, method="fdr_bh")
            selected_pathways = len(y[1][np.where(y[1]<alpha)])

            # CREATE NOISE
            fdrA = np.random.normal(0, alpha, len(FDR_count_random))
            fdrB = np.random.normal(0, alpha, len(FDR_count_random))
            fdrC = np.random.normal(0, alpha, len(FDR_count_random))
            fdrD = np.random.normal(0, alpha, len(FDR_count_random))
            fdrE = np.random.normal(0, alpha, len(FDR_count_random))
            fdrF = np.random.normal(0, alpha, len(FDR_count_random))

            # PLOT FOR FDR_COUNT REGARDING FISHER_EXACT TEST
            ax[phenotype, geneset].plot(np.arange(len(FDR_count_random)), FDR_count_SV_ordering + fdrA[:], '.', label = 'SV') 
            ax[phenotype, geneset].plot(np.arange(len(FDR_count_random)), FDR_count_ordering_normal + fdrB[:], '.', label = 'PO') 
            ax[phenotype, geneset].plot(np.arange(len(FDR_count_random)), FDR_count_ordering_normal_rescaled + fdrC[:], '.', label = 'POR') 
            ax[phenotype, geneset].plot(np.arange(len(FDR_count_random)), FDR_count_ordering_artificial + fdrD[:], '.', label = 'AO') 
            ax[phenotype, geneset].plot(np.arange(len(FDR_count_random)), FDR_count_ordering_artificial_rescaled + fdrE[:], '.', label = 'AOR')
